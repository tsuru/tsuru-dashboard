...
from apps.views import AutoscaleDisable

from mock import patch


class AutoscaleDisableTest(TestCase):
    ...
    def test_method_not_allowed(self):
        ...
        for method in methods:
            ...
            response = AutoscaleDisable.as_view()(request, app_name="app1")
            self.assertEqual(response.status_code, 405)

    @patch("requests.put")
    def test_disable(self, put_mock):
        app_name = "app1"

        request = RequestFactory().post("/")
        request.session = {"tsuru_token": "admin"}

        response = AutoscaleDisable.as_view()(request, app_name=app_name)
