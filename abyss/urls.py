...
from apps.views import (CreateApp, AppAddTeam, Run, ListApp, RemoveApp, AppLog,
                        AppDetail, AppEnv, AppTeams)
...
urlpatterns = patterns(
    ...
    url(r'^app/(?P<app_name>[\w-]+)/$',
        AppDetail.as_view(), name='detail-app'),
    url(r'^app/(?P<name>[\w-]+)/remove/$',
        RemoveApp.as_view(), name='remove_app'),
    url(r'^app/(?P<app_name>[\w-]+)/log/$', AppLog.as_view(), name='app_log'),
    url(r'^app/(?P<app_name>[\w-]+)/env/$', AppEnv.as_view(), name='get-env'),
    url(r'^app/(?P<app_name>[\w-]+)/teams/$',
